...
local function Move()
	local val = 100
	local Reboundspeed = 0.4
	local ReboundDelay = 0.6
	local storer = Reboundspeed
	local entityheight = Vector3.new(0,2,0)

	local s= game:GetObjects("rbxassetid://12815662543")[1]
	s.Parent = workspace
	...
	spawn(function()
		while entity ~= nil do wait(0.5)
			...
			if v.Character ~= nil and not v.Character:GetAttribute("Hiding") then
				if canSeeTarget(v.Character,50) and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").MoveDirection ~= Vector3.new(0,0,0) then
					--ReboundMoving:Stop()
					firesignal(game.ReplicatedStorage.EntityInfo.DeathHint.OnClientEvent, {"You died to Fester","He hates Motion senses!","Try to avoid moving while you can!"},"Blue")
					game:GetService("ReplicatedStorage").GameStats["Player_".. game.Players.LocalPlayer.Name].Total.DeathCause.Value = "Fester"
					wait(.5)
